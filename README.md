...
- NSFW 标签分类及开关控制
- 🖼 **标签可视化**：每个标签显示为缩略图卡片（3:4 肖像比例），封面为最近生成图片；支持点击🔍查看全部关联图片
- 💾 **标签图片持久化**：生图时自动保存图片到服务端独立存储，返图区/相册清空不影响标签封面
- ⚙ **卡片上限设置**：工具栏「⚙ 卡片」可自定义每标签保存图片数（1~20），缩小上限自动清理旧图
- 📋 **浏览全部标签**：工具栏「📋 浏览全部」弹窗，全屏多列网格展示当前子类全部标签，支持鼠标拖拽批量勾选 + 一键删除
...
- 🧩 **防冲突规则编辑器**：可视化设置大类全局互斥、子类互斥、同组共享
