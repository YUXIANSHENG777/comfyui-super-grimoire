...
function _queueWithRefine(prompt,wf,w,h,count){if(el('llm-auto-refine').checked){var llmInput=_getLlmPrompt();if(!llmInput)llmInput=prompt;var raw=_buildRawPrompt(),negPart='';if(raw.indexOf('--neg ')>=0)negPart='\n--neg '+raw.split('--neg ')[1];el('comfyui-result').innerHTML='⏳ AI润色中...';el('comfyui-result').style.color='var(--text-muted)';_callLlm(llmInput,function(text){if(text){prompt=text;if(S.template)prompt=S.template.replace(/{tags}/g,prompt);if(negPart)prompt+=negPart;S.llmHistory.unshift(prompt);if(S.llmHistory.length>100)S.llmHistory.pop();saveLlmHistory();}_queueCuiPrompt(prompt,wf,w,h,count);toast('已添加(已润色) '+count+' 个任务');});}else{_queueCuiPrompt(prompt,wf,w,h,count);toast('已添加 '+count+' 个任务到队列');}}
