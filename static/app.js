...
el('btn-llm-send').addEventListener('click',function(){var prompt=el('prompt-output').value;if(!prompt.trim()){toast('提示词为空');return;}el('llm-result').textContent='⏳ 生成中...';_callLlm(prompt,function(text){if(text){var r=text;if(S.template)r=S.template.replace(/{tags}/g,r);S.llmHistory.unshift(r);if(S.llmHistory.length>100)S.llmHistory.pop();saveLlmHistory();el('llm-result').textContent=r;el('llm-result').style.color='var(--text-primary)';}else{el('llm-result').textContent='❌ 请求失败';el('llm-result').style.color='var(--danger)';}});});
