...
// 画廊爱心按钮 - 保存/取消 切换
function _saveToAlbum(gi){if(!gi||!gi.url)return false;var album=null;for(var i=0;i<_albumData.length;i++)if(_albumData[i].id===_activeAlbumId){album=_albumData[i];break;}if(!album)return false;var fn=gi.filename||'';if(fn){for(var i=0;i<album.images.length;i++)if(album.images[i].filename===fn)return false;}else{for(var i=0;i<album.images.length;i++)if(album.images[i].url===gi.url)return false;}album.images.push({url:gi.url,filename:gi.filename||'',prompt:gi.prompt||'',path:gi.path||''});_saveAlbums();_renderAlbumTabs();_renderAlbumCard();return true;}
