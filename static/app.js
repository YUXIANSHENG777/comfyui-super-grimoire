/* ========== 超级无敌魔导书 - 应用逻辑 ========== */
// 核心依赖在 app-core.js 中加载：ICONS, icon, S, QW, el, qs, qsa, esc, toast, api, 存储函数等
...
html+='<button class=tag-lock'+(t.locked?' locked':'')+' data-en="'+esc(t.en)+'" data-panel="'+panel+'" title='+(t.locked?'已锁定不刷新':'点击锁定不刷新')+'>'+(t.locked?'<span class="svg-icon"><svg viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" fill="currentColor" opacity="0.2"/><path d="M7 11V7a5 5 0 0 1 10 0v4" fill="currentColor"/></svg></span>':'<span class="svg-icon"><svg viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg></span>')+'</button>';
...
var cm=S.catModes[c.name]==='cat'?'cat':'sc';html+='<button class=cat-mode-btn data-cat='+esc(c.name)+' title='+(cm==='cat'?'按分类随机':'按子类随机')+'>'+(cm==='cat'?'<span class="svg-icon"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg></span> 分类':'<span class="svg-icon"><svg viewBox="0 0 24 24"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg></span> 子类')+'</button>';
...
function loadHistory(){api('/api/history').then(function(items){var html='';if(items&&items.length>0){
  ...
  for(var i=0;i<Math.min(items.length,50);i++){var h=items[i];var raw=h.prompt||'';var p=raw.length>60?raw.substring(0,60)+'...':raw;html+='<div class=history-item data-id='+esc(h._filename)+' title='+esc(h.created)+'><span style=flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10px>'+esc(p)+'</span><button class=hist-del-btn data-id='+esc(h._filename)+' style=\"border:none;background:none;color:var(--text-muted);cursor:pointer;font-size:12px;padding:2px 4px\">✕</button></div>';}
...
// 服务端同步
var _syncDirty={};var _syncTimer=null;
function _syncSave(keys){
  if(!keys){_syncFlush();return;}
  for(var k in keys)_syncDirty[k]=1;
  if(_syncTimer)clearTimeout(_syncTimer);
  _syncTimer=setTimeout(_syncFlush,500);
}
function _syncFlush(){
  _syncTimer=null;
  var keys=Object.keys(_syncDirty);
  if(!keys.length){
    // 全量同步
    var d={};
    try{var favs=S.favs;if(Object.keys(favs).length)d.favs=favs;}catch(e){}
    try{var cl=S.catLimits;if(Object.keys(cl).length)d.catLimits=cl;}catch(e){}
    try{var sl=S.scLimits;if(Object.keys(sl).length)d.scLimits=sl;}catch(e){}
    try{var cm=S.catModes;if(Object.keys(cm).length)d.catModes=cm;}catch(e){}
    d.allowNsfw=S.allowNsfw;d.randWeight=S.randWeight;d.spaceMode=S.spaceMode;d.tagMode=S.tagMode;
    try{var tl=JSON.parse(localStorage.getItem('grimoire2_templates')||'[]');if(tl.length)d.templates=tl;}catch(e){}
    try{if(Object.keys(S.wfSettings||{}).length)d.wfSettings=S.wfSettings;}catch(e){}
    try{if(Object.keys(S.clipMaps||{}).length)d.clipMaps=S.clipMaps;}catch(e){}
    try{var ms=JSON.parse(localStorage.getItem('grimoire2_modelsel')||'{}');if(Object.keys(ms).length)d.modelSel=ms;}catch(e){}
    try{if(S.llmHistory&&S.llmHistory.length)d.llmHist=S.llmHistory;}catch(e){}
    try{if(_galleryImages.length)d.gallery=_galleryImages;}catch(e){}
    try{var nt=JSON.parse(localStorage.getItem('grimoire2_negtpl_list')||'[]');if(nt.length)d.negtpl_list=nt;}catch(e){}
    try{var np=JSON.parse(localStorage.getItem('grimoire2_negtpl')||'{}');if(Object.keys(np).length)d.negtpl=np;}catch(e){}
    try{var _li=S.loadImage.filter(function(x){return x.node_id&&x.filename;});if(_li.length)d.loadImg=S.loadImage;}catch(e){}
    try{var rp=JSON.parse(localStorage.getItem('grimoire2_randprof')||'[]');if(rp.length)d.randprof=rp;}catch(e){}
    try{var ui=JSON.parse(localStorage.getItem('grimoire2_ui')||'{}');if(Object.keys(ui).length)d.uiSettings=ui;}catch(e){}
    try{var ad=JSON.parse(localStorage.getItem('grimoire2_albums')||'[]');if(ad.length)d.albums={data:ad,activeId:localStorage.getItem('grimoire2_active_album')||'default'};}catch(e){}
    try{var lk=JSON.parse(localStorage.getItem('grimoire2_locked')||'{}');if(Object.keys(lk).length)d.locked=lk;}catch(e){}
    try{if(Object.keys(S.hidden||{}).length)d.hidden=S.hidden;}catch(e){}
    if(Object.keys(d).length)api('/api/user/sync',{method:'POST',body:d});
    return;
  }
  _syncDirty={};
  var keysObj={};keys.forEach(function(k){keysObj[k]=1;});
  var d={};
  function add(k,v){if(keysObj[k]){if(typeof v==='object'&&v!==null){if(Array.isArray(v)){if(v.length)d[k]=v;}else{if(Object.keys(v).length)d[k]=v;}}else if(v!==undefined&&v!==null&&v!==''){d[k]=v;}}}
  add('favs',S.favs);add('catLimits',S.catLimits);add('scLimits',S.scLimits);
  add('catModes',S.catModes);add('allowNsfw',S.allowNsfw);add('randWeight',S.randWeight);
  add('spaceMode',S.spaceMode);add('tagMode',S.tagMode);
  try{var tl=JSON.parse(localStorage.getItem('grimoire2_templates')||'[]');add('templates',tl);}catch(e){}
  add('wfSettings',S.wfSettings||{});add('clipMaps',S.clipMaps||{});
  try{add('modelSel',JSON.parse(localStorage.getItem('grimoire2_modelsel')||'{}'));}catch(e){}
  add('llmHist',S.llmHistory||[]);add('gallery',_galleryImages);
...
function _syncLoad(cb){
  api('/api/user/sync').then(function(d){
    ...
    if(d.loadImg){if(Array.isArray(d.loadImg)){S.loadImage=d.loadImg;localStorage.setItem('grimoire2_loadimg',JSON.stringify(d.loadImg));}else if(d.loadImg.node_id){var _old=d.loadImg;S.loadImage=[{node_id:_old.node_id,filename:_old.filename},{node_id:null,filename:null},{node_id:null,filename:null}];localStorage.setItem('grimoire2_loadimg',JSON.stringify(S.loadImage));}}
...
var _searchTimer;el('search-input').addEventListener('input',function(){var q=this.value.trim();var b=el('btn-search-clear');if(q){b.style.display='block';clearTimeout(_searchTimer);_searchTimer=setTimeout(function(){doSearch(q);},300);}else{b.style.display='none';clearTimeout(_searchTimer);clearSearch();}});
...
// 加载图片功能（3插槽）
S.loadImage=[{node_id:null,filename:null},{node_id:null,filename:null},{node_id:null,filename:null}];
try{var _li=JSON.parse(localStorage.getItem('grimoire2_loadimg')||'null');if(_li){if(Array.isArray(_li)&&_li.length===3)S.loadImage=_li;else if(_li.node_id)S.loadImage=[{node_id:_li.node_id,filename:_li.filename},{node_id:null,filename:null},{node_id:null,filename:null}];}}catch(e){}
function _saveLoadImage(){localStorage.setItem('grimoire2_loadimg',JSON.stringify(S.loadImage));}
function _fillLoadImgSlots(nodes){for(var i=0;i<3;i++){var sel=el('loadimg-node-'+i);sel.innerHTML='<option value="">-- 节点 --</option>';for(var j=0;j<nodes.length;j++){sel.innerHTML+='<option value="'+nodes[j].id+'">节点 '+nodes[j].id+(nodes[j].title?' ('+nodes[j].title+')':'')+'</option>';}if(S.loadImage[i]&&S.loadImage[i].node_id)sel.value=S.loadImage[i].node_id;var st=el('loadimg-status-'+i);if(S.loadImage[i]&&S.loadImage[i].filename){st.textContent='✅ '+S.loadImage[i].filename;st.style.color='var(--success)';}else{st.textContent='';}}}
function _doUploadImageSlot(file,idx){
  var reader=new FileReader();
  reader.onload=function(e){el('loadimg-preview-img-'+idx).src=e.target.result;el('loadimg-preview-'+idx).style.display='block';};
  reader.readAsDataURL(file);
  el('loadimg-status-'+idx).textContent='上传中...';el('loadimg-status-'+idx).style.color='var(--text-muted)';
  ...
  fetch('/api/comfyui/upload-image',{method:'POST',body:formData}).then(function(r){return r.json();}).then(function(r){
    if(r.ok){S.loadImage[idx]={node_id:el('loadimg-node-'+idx).value||null,filename:r.filename};_saveLoadImage();el('loadimg-status-'+idx).textContent='✅ '+r.filename;el('loadimg-status-'+idx).style.color='var(--success)';}else{el('loadimg-status-'+idx).textContent='失败: '+(r.error||'未知');el('loadimg-status-'+idx).style.color='var(--danger)';}
  });
}
// 拖拽上传（桌面端）
...
_loadImgBtn.addEventListener('drop',function(e){
  e.preventDefault();e.stopPropagation();this.style.borderColor='';this.style.background='';
  var file=e.dataTransfer.files[0];if(!file||!file.type.startsWith('image/')){toast('请拖入图片文件');return;}
  var wf=el('comfyui-workflow').value;if(!wf){toast('请先选择工作流');return;}
  el('modal-loadimg').style.display='';
  api('/api/comfyui/loadimage-nodes?file='+encodeURIComponent(wf)).then(function(r){_fillLoadImgSlots(r.nodes||[]);});
  for(var si=0;si<3;si++){if(!S.loadImage[si]||!S.loadImage[si].filename){_doUploadImageSlot(file,si);return;}}
  _doUploadImageSlot(file,0);
});
...
for(var si=0;si<3;si++){(function(idx){
  el('btn-loadimg-upload-'+idx).addEventListener('click',function(){var file=el('loadimg-file-'+idx).files[0];if(!file){toast('请先选择图片');return;}_doUploadImageSlot(file,idx);});
  el('loadimg-file-'+idx).addEventListener('change',function(){var f=this.files[0];if(!f)return;var reader=new FileReader();reader.onload=function(e){el('loadimg-preview-img-'+idx).src=e.target.result;el('loadimg-preview-'+idx).style.display='block';};reader.readAsDataURL(f);});
})(si);}
el('btn-loadimg-close').addEventListener('click',function(){el('modal-loadimg').style.display='none';});
el('btn-loadimg-clear').addEventListener('click',function(){for(var i=0;i<3;i++){S.loadImage[i]={node_id:null,filename:null};el('loadimg-status-'+i).textContent='';el('loadimg-preview-'+i).style.display='none';el('loadimg-file-'+i).value='';}_saveLoadImage();toast('已清除全部图片绑定');});
...
// 首次加载注入默认反推预设
(function(){
  if(localStorage.getItem('grimoire2_reverse_seeded')==='v4')return;
  localStorage.removeItem('grimoire2_reverse_presets');
  localStorage.setItem('grimoire2_reverse_seeded','v4');
  var defs=[
    {name:'🔍 通用反推',sys:'你是一个AI绘画提示词反推专家。仔细观察这张图片，输出一段可直接用于Stable Diffusion生图的英文提示词。按从整体到细节的顺序描述：画面内容与场景、人物外貌与服装、表情与动作、光线氛围与色彩、构图视角、艺术风格与画质。描述要具体、可执行，写出材质、光线、色彩、空间和景深的具体细节。完整一段英文，150词左右，末尾附【镜头：焦距_光圈_景深_对焦_背景处理】。只输出提示词。'},
    {name:'🎌 转二次元/动漫',sys:'你是一个AI绘画风格转换专家。分析这张图片的内容（人物、场景、构图、氛围），然后用动漫/二次元风格的英文词汇重新描述。保留原图的核心内容和构图，但将风格转换为anime style, 2D illustration, cel shading, clean linework等动漫视觉风格。不保留任何写实摄影词汇。完整一段英文。只输出提示词。'},
    {name:'📸 转真人写实',sys:'你是一个AI绘画风格转换专家。分析这张图片的内容，然后用真人写实/电影摄影风格的英文词汇重新描述。保留原图的核心内容和构图，但将所有元素转换为真实物理世界描述，使用photorealistic, 8K, detailed skin texture, professional photography等风格词。不保留任何动漫/二次元词汇。完整一段英文。只输出提示词。'},
    {name:'🎨 转油画/手绘',sys:'你是一个AI绘画风格转换专家。分析这张图片的内容，然后用传统油画/手绘风格的英文词汇重新描述。保留原图的核心内容和构图，但转换为传统艺术风格，使用oil painting, impasto, canvas texture, brush strokes, traditional media等词汇。完整一段英文。只输出提示词。'},
    {name:'🌃 转赛博朋克',sys:'你是一个AI绘画风格转换专家。分析这张图片的内容，保留人物的基本关系和构图骨架，但将所有视觉元素风格化为赛博朋克世界。用英文描述，融入cyberpunk, neon lights, rain-soaked streets, holographic displays, high-tech, dystopian等元素。完整一段英文。只输出提示词。'},
    {name:'📝 中文反推',sys:'仔细观察这张图片，用中文写一段流畅的画面描述，200字左右。按顺序描述：整体场景→人物细节→服装材质→光线色彩→构图氛围→艺术风格。用具体的、可见的词汇，不要评价。只输出描述。'}
  ];
...
el('btn-reverse-generate').addEventListener('click',function(){
  var slot=parseInt(el('reverse-slot').value)||0;
  var file=el('loadimg-file-'+slot).files[0];
  if(!file){toast('请先在插槽 #'+(slot+1)+' 上传图片');return;}
...
el('btn-comfyui-size').addEventListener('click',function(){_openResolutionPicker();});
...
function loadLlmConfig(){
  ...
  api('/api/llm/config').then(function(srv){
    ...
    if(s){
      ...
      el('llm-lang').value=s.lang||'en';el('llm-concurrent').checked=s.allowCon||false;if(el('llm-unload'))el('llm-unload').checked=s.unload||false;
    }
  }).catch(function(){
...
function _unloadLLM(){var cb=el('llm-unload');if(!cb||!cb.checked)return;var prov=el('llm-provider').value;var base=el('llm-url').value.replace(/\/v1\/?$/,'').replace(/\/$/,'');if(prov==='ollama'&&base){fetch(base+'/api/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:el('llm-model').value||'qwen2.5','keep_alive':'0m'})}).catch(function(){});}else if(prov==='lmstudio'&&base){var mn=el('llm-model').value||'model';api('/api/llm/unload-model',{method:'POST',body:{instance_id:mn,base_url:base}}).catch(function(){});}}
function saveLlmConfig(){
  var s={prov:el('llm-provider').value,url:el('llm-url').value,model:el('llm-model').value,key:el('llm-key').value,sys:el('llm-sysprompt').value,auto:el('llm-auto-refine').checked,lang:el('llm-lang').value,allowCon:el('llm-concurrent').checked,unload:el('llm-unload')?el('llm-unload').checked:false};
...
function _openResolutionPicker(){var portrait=[{w:512,h:768,l:'2:3'},{w:576,h:768,l:'3:4'},{w:512,h:910,l:'9:16'},{w:768,h:1024,l:'3:4'},{w:832,h:1216,l:'2:3'},{w:768,h:1344,l:'9:16'},{w:640,h:1536,l:'9:21'},{w:1024,h:1280,l:'4:5'},{w:1024,h:1536,l:'2:3'},{w:1080,h:1920,l:'9:16'},{w:1280,h:1707,l:'3:4'},{w:1440,h:2560,l:'9:16 2K'}];var selW=el('comfyui-width');var selH=el('comfyui-height');var cw=parseInt(selW.value)||512,ch=parseInt(selH.value)||512;var html='<div style="font-size:11px;color:var(--text-muted);margin-bottom:10px">当前: <b>'+cw+'×'+ch+'</b></div>';html+='<div style="display:flex;gap:6px;margin-bottom:10px"><button class="mini-btn active" id="res-tab-portrait" onclick="_resSwitchTab(\'portrait\')">📱 竖屏</button><button class="mini-btn" id="res-tab-square" onclick="_resSwitchTab(\'square\')">⬜ 方图</button><button class="mini-btn" id="res-tab-landscape" onclick="_resSwitchTab(\'landscape\')">🖥 横屏</button></div>';html+='<div id="res-grid-container"><div class="resolution-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(100px,1fr));gap:6px">';portrait.forEach(function(r){var act=cw===r.w&&ch===r.h?' active':'';html+='<div class="res-item'+act+'" onclick="el(\'comfyui-width\').value='+r.w+';el(\'comfyui-height\').value='+r.h+';saveUiSettings();el(\'modal-resolution\').style.display=\'none\';toast(\'已设为 '+r.w+'×'+r.h+'\')"><div style="font-weight:600">'+r.w+'×'+r.h+'</div><div style="font-size:9px;color:var(--text-muted)">'+r.l+'</div></div>';});html+='</div></div>';html+='<div style="margin-top:10px;display:flex;gap:6px"><button class="mini-btn" onclick="el(\'modal-resolution\').style.display=\'none\'" style="font-size:11px">关闭</button></div>';el('resolution-picker-body').innerHTML=html;el('modal-resolution').style.display='';}
function _resSwitchTab(tab){['portrait','square','landscape'].forEach(function(t){var btn=el('res-tab-'+t);if(btn)btn.classList.toggle('active',t===tab);});var grid=el('res-grid-container');if(!grid)return;var data={portrait:[{w:512,h:768,l:'2:3'},{w:576,h:768,l:'3:4'},{w:512,h:910,l:'9:16'},{w:768,h:1024,l:'3:4'},{w:832,h:1216,l:'2:3'},{w:768,h:1344,l:'9:16'},{w:640,h:1536,l:'9:21'},{w:720,h:1280,l:'9:16'},{w:1024,h:1280,l:'4:5'},{w:1024,h:1536,l:'2:3'},{w:1080,h:1920,l:'9:16'},{w:1280,h:1707,l:'3:4'},{w:1440,h:2560,l:'9:16 2K'}],landscape:[{w:768,h:512,l:'3:2'},{w:768,h:576,l:'4:3'},{w:910,h:512,l:'16:9'},{w:1024,h:768,l:'4:3'},{w:1216,h:832,l:'3:2'},{w:1344,h:768,l:'16:9'},{w:1536,h:640,l:'21:9'},{w:1280,h:1024,l:'5:4'},{w:1536,h:1024,l:'3:2'},{w:1280,h:720,l:'16:9'},{w:1920,h:1080,l:'16:9'},{w:1707,h:1280,l:'4:3'},{w:2560,h:1440,l:'16:9 2K'}],square:[{w:512,h:512,l:'1:1'},{w:768,h:768,l:'1:1'},{w:1024,h:1024,l:'1:1'},{w:1280,h:1280,l:'1:1'},{w:1536,h:1536,l:'1:1'},{w:2048,h:2048,l:'1:1 2K'}]};var items=data[tab]||[];var html='<div class="resolution-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(100px,1fr));gap:6px">';items.forEach(function(r){var act=parseInt(el('comfyui-width').value)===r.w&&parseInt(el('comfyui-height').value)===r.h?' active':'';html+='<div class="res-item'+act+'" onclick="el(\'comfyui-width\').value='+r.w+';el(\'comfyui-height\').value='+r.h+';saveUiSettings();el(\'modal-resolution\').style.display=\'none\';toast(\'已设为 '+r.w+'×'+r.h+'\')"><div style="font-weight:600">'+r.w+'×'+r.h+'</div><div style="font-size:9px;color:var(--text-muted)">'+r.l+'</div></div>';});html+='</div>';grid.innerHTML=html;}
var S_cuiSel={};
var _ICK={checkpoint:'<span class="svg-icon"><svg viewBox="0 0 24 24"><polygon points="12 2 2 7 12 12 22 7 12 2"/><polyline points="2 17 12 22 22 17"/><polyline points="2 12 12 17 22 12"/></svg></span> Checkpoint',unet:'<span class="svg-icon"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg></span> UNET',clip:'<span class="svg-icon"><svg viewBox="0 0 24 24"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg></span> CLIP',vae:'<span class="svg-icon"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg></span> VAE',dual_clip:'<span class="svg-icon"><svg viewBox="0 0 24 24"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg></span> DualCLIP',lora:'<span class="svg-icon"><svg viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg></span> LoRA'};
...
function fetchModels(){var f=el('comfyui-workflow').value;if(!f)return;el('model-search-input').value='';api('/api/comfyui/workflow-info?file='+encodeURIComponent(f)).then(function(info){api('/api/comfyui/models').then(function(m){try{_renderModelNodes(info,m);}catch(e){el('modal-models-body').innerHTML='<div style=font-size:11px;color:var(--danger);padding:20px;text-align:center>渲染错误: '+esc(e.message)+'</div>';}}).catch(function(){_renderModelNodes(info,{});});}).catch(function(e){el('modal-models-body').innerHTML='<span style=font-size:11px;color:var(--danger)>工作流解析失败: '+(e.error||'未知')+'</span>';});}
...
function _loadFmTree(){
  var bp=[];try{bp=JSON.parse(localStorage.getItem('grimoire2_bp')||'[]');}catch(e){}
  if(!bp.length||!bp[0]){el('fm-tree').innerHTML='<div style=font-size:10px;color:var(--text-muted);padding:10px>请先填写路径</div>';return;}
  api('/api/bind/subdirs',{method:'POST',body:{paths:bp}}).then(function(r){
    var html='';var firstDir='';
    for(var p in r){
      html+='<div style=font-size:10px;color:var(--text-muted);padding:4px 6px;font-weight:600;border-bottom:1px solid var(--border);margin-bottom:2px>'+esc(p.split('\\').pop()||p)+'</div>';
      var dirs=r[p]||[];
      for(var i=0;i<dirs.length;i++){
        var d=dirs[i];
        if(!firstDir&&d.count>0)firstDir=d.path;
        var act=_fmData.curSubdir===d.path?' background:var(--accent-glow);color:var(--accent)':'';
        var icon=d.root?'📂':'📁';
        html+='<div class=fm-dir data-path="'+esc(d.path)+'" style="cursor:pointer;padding:3px 6px;border-radius:3px;font-size:10px;margin-bottom:1px'+act+'" title="'+d.count+' 张">'+icon+' '+esc(d.name)+' <span style=color:var(--text-muted);font-size:9px>('+d.count+')</span></div>';
      }
    }
    el('fm-tree').innerHTML=html;
    el('fm-tree').querySelectorAll('.fm-dir').forEach(function(el){
      el.addEventListener('click',function(){_fmData.curSubdir=this.dataset.path;_fmData.images=[];_fmData.selected={};_loadFmImages();});
    });
    if(!_fmData.curSubdir&&firstDir){_fmData.curSubdir=firstDir;_loadFmImages();}
    el('fm-info').textContent=_fmData.curSubdir?'当前: '+_fmData.curSubdir:'';
  });
}
function _loadFmImages(){
  var bp=[];try{bp=JSON.parse(localStorage.getItem('grimoire2_bp')||'[]');}catch(e){}
  if(!bp.length||!_fmData.curSubdir){el('fm-grid-wrap').innerHTML='<div style=font-size:10px;color:var(--text-muted);width:100%;text-align:center;padding:40px>请选择目录</div>';return;}
  el('fm-grid-wrap').innerHTML='<div style=font-size:10px;color:var(--text-muted);width:100%;text-align:center;padding:40px>⏳ 加载中...</div>';
  var base=bp[0],rel='';if(_fmData.curSubdir.startsWith(base))rel=_fmData.curSubdir.substring(base.length).replace(/^[\\\/]/,'');
  api('/api/bind/scan',{method:'POST',body:{paths:[base],subfolder:rel}}).then(function(r){
    _fmData.images=r.images||[];_fmData.selected={};_renderFmGrid();
  });
}
function _renderFmGrid(){
  var q=el('bind-file-search').value.trim().toLowerCase();
  var grid=el('fm-grid-wrap');grid.innerHTML='';
  var imgs=_fmData.images;
  if(q)imgs=imgs.filter(function(x){return x.filename.toLowerCase().indexOf(q)>=0;});
  if(!imgs.length){grid.innerHTML='<div style=font-size:10px;color:var(--text-muted);width:100%;text-align:center;padding:40px>无图片</div>';return;}
  for(var i=0;i<imgs.length;i++){
    (function(gi,idx){
      var wrap=document.createElement('div');wrap.style.cssText='width:100px;height:100px;border-radius:4px;overflow:hidden;border:2px solid var(--border);cursor:pointer;position:relative;background:var(--bg-tertiary)';
      var img=document.createElement('img');img.dataset.src=gi.url;img.style.cssText='width:100%;height:100%;object-fit:cover';
      img.onerror=function(){this.parentElement.style.display='none';};
      var obs=new IntersectionObserver(function(es){es.forEach(function(e){if(e.isIntersecting){if(img.dataset.src&&!img.src){img.src=img.dataset.src;}obs.unobserve(img);}});},{rootMargin:'200px'});
      obs.observe(img);
      img.addEventListener('click',function(e){e.stopPropagation();_openImgPreview(gi.url,gi.filename||'');});
      var cb=document.createElement('input');cb.type='checkbox';cb.style.cssText='position:absolute;top:3px;left:3px;z-index:2;cursor:pointer;margin:0';
      cb.checked=!!_fmData.selected[gi.url];
      cb.addEventListener('change',function(){if(this.checked)_fmData.selected[gi.url]=true;else delete _fmData.selected[gi.url];_updateFmToolbar();});
      wrap.appendChild(img);wrap.appendChild(cb);grid.appendChild(wrap);
    })(imgs[i],i);
  }
  _updateFmToolbar();
}
function _updateFmToolbar(){
  var sel=Object.keys(_fmData.selected).length;
  el('fm-info').textContent=(_fmData.curSubdir?'📂 '+_fmData.curSubdir.split('\\').pop()+' | ':'')+_fmData.images.length+' 张'+(sel?' | 已选 '+sel+' 张':'');
}
function _fmLoadImages(urls){
  if(!urls||!urls.length)return;
  // 按 filename 去重（兼容 URL 不同但文件名相同的情况）
  function _getKey(obj){return obj.filename||obj.url||'';}
  // 收集相册已有的 key
  var albumKeys={};for(var j=0;j<_albumData.length;j++){for(var k=0;k<_albumData[j].images.length;k++){albumKeys[_getKey(_albumData[j].images[k])]=true;}}
  // 收集返图区已有的 key
  var existKeys={};for(var j=0;j<_galleryImages.length;j++){existKeys[_getKey(_galleryImages[j])]=true;}
  var added=0,skippedAlbum=0,skippedExist=0;
  for(var i=0;i<urls.length;i++){
    if(!urls[i])continue;
    // 从 _fmData.images 中找到完整数据
    var src=null;for(var k=0;k<_fmData.images.length;k++){if(_fmData.images[k].url===urls[i]){src=_fmData.images[k];break;}}
    if(!src)continue;
    var key=_getKey(src);
    if(albumKeys[key]){skippedAlbum++;continue;}
    if(existKeys[key]){skippedExist++;continue;}
    _galleryImages.push(src);existKeys[key]=true;added++;
  }
  if(!added){var msg='没有新图片需要加载';if(skippedAlbum)msg+=' ('+skippedAlbum+' 张已在相册)';if(skippedExist)msg+=' ('+skippedExist+' 张已在返图区)';toast(msg);return;}
  _saveGallery();
  var ia=el('comfyui-gallery-imgs');ia.innerHTML='';var frag=document.createDocumentFragment();
  for(var i=0;i<_galleryImages.length;i++){(function(gi){
    var w=document.createElement('div');w.className='gallery-img-wrap';var im=document.createElement('img');im.dataset.src=gi.url;im.style.background='var(--bg-tertiary)';
    im.onerror=function(){this.parentElement.style.display='none';};_observeGalImg(im);
    im.addEventListener('click',function(){_openImgPreview(this.dataset.src||this.src,gi.filename||'');});
    ...
    inf.appendChild(lb);w.appendChild(im);w.appendChild(inf);_addGalleryHeart(w,gi);_addDelBtn(w,gi);
    if(_isImageInActiveAlbum(gi.url,gi.filename))w.style.display='none';frag.appendChild(w);
  })(_galleryImages[i]);}
  ia.appendChild(frag);_updateGalleryCount();
  var msg='已加载 '+added+' 张新图片到返图区';
  if(skippedAlbum)msg+='，跳过 '+skippedAlbum+' 张（已在相册）';
  if(skippedExist)msg+='，跳过 '+skippedExist+' 张（已在返图区）';
  toast(msg);
}
// 打开/关闭
el('btn-bind-path').addEventListener('click',function(){var bp=[];try{bp=JSON.parse(localStorage.getItem('grimoire2_bp')||'[]');}catch(e){}el('bind-path-1').value=bp[0]||'';el('bind-path-2').value=bp[1]||'';el('bind-file-search').value='';_fmData.curSubdir='';_fmData.images=[];_fmData.selected={};el('modal-bind-path').style.display='';_loadFmTree();});
el('btn-bind-close').addEventListener('click',function(){el('modal-bind-path').style.display='none';});
el('modal-bind-path').addEventListener('click',function(e){if(e.target===this)this.style.display='none';});
el('btn-bind-scan').addEventListener('click',function(){var p1=el('bind-path-1').value.trim();var p2=el('bind-path-2').value.trim();var bp=[];if(p1)bp.push(p1);if(p2)bp.push(p2);localStorage.setItem('grimoire2_bp',JSON.stringify(bp));if(!bp.length){toast('请填写路径');return;}_fmData.curSubdir='';_loadFmTree();});
el('btn-fm-load').addEventListener('click',function(){var sel=Object.keys(_fmData.selected);if(!sel.length){toast('请先勾选要加载的图片');return;}_fmLoadImages(sel);});
el('btn-fm-load-all').addEventListener('click',function(){_fmLoadImages(_fmData.images.map(function(x){return x.url;}));});
el('bind-file-search').addEventListener('input',function(){_renderFmGrid();});
// 全部预览
...
// ===== 大图预览 + 虚拟滚动(懒加载) =====
// 全局 IntersectionObserver 实现 gallery 图片懒加载
var _galLazyObs=null;
function _initGalLazyObs(){if(_galLazyObs)return;_galLazyObs=new IntersectionObserver(function(entries){entries.forEach(function(e){if(e.isIntersecting){var img=e.target;if(img.dataset.src&&!img.src){img.src=img.dataset.src;}_galLazyObs.unobserve(img);}});},{rootMargin:'400px'});}
function _observeGalImg(img){if(!_galLazyObs)_initGalLazyObs();if(img.dataset.src)_galLazyObs.observe(img);}
_initGalLazyObs();
  ...
  try{var d=JSON.parse(localStorage.getItem('grimoire2_gallery')||'[]');_galleryImages=d||[];
    if(_galleryImages.length){
      ...
      for(var i=0;i<_galleryImages.length;i++){
        ...
        var elm=document.createElement('img');elm.dataset.src=gi.url;elm.style.background='var(--bg-tertiary)';
        elm.onerror=function(){this.parentElement.style.display='none';};_observeGalImg(elm);
        elm.addEventListener('click',(function(u,f){return function(){_openImgPreview(u||this.src,f);};})(gi.url,fn));
        var info=document.createElement('div');info.className='gallery-img-info';
        var lbl=document.createElement('span');lbl.className='gallery-img-label';lbl.textContent=fn;
...
function _openImgPreview(url,label){
  ...
  _previewImageData=_galleryImages[idx]||null;_previewAlbum=null;
  _loadPreviewImage(url);
  el('img-preview-modal').classList.add('active');_updatePreviewNav();
  var gi=_galleryImages[idx];
  var raw=gi.prompt||'';
  if(raw){var ni=raw.indexOf('\n--neg ');if(ni<0)ni=raw.indexOf('--neg ');el('img-preview-meta-text').textContent=ni>=0?raw.substring(0,ni).trim():raw.trim();}
  else if(gi.path){
    el('img-preview-meta-text').textContent='⏳ 读取中...';
    api('/api/bind/meta',{method:'POST',body:{path:gi.path}}).then(function(d){
      if(d&&d.ok){var txt=d.positive||'';gi.prompt=txt.trim();_galleryImages[idx]=gi;localStorage.setItem('grimoire2_gallery',JSON.stringify(_galleryImages));el('img-preview-meta-text').textContent=txt.trim()||'（无）';}
      else el('img-preview-meta-text').textContent='（无）';
    }).catch(function(){el('img-preview-meta-text').textContent='（无）';});
  }
  else el('img-preview-meta-text').textContent='（无）';
...
function _previewNext(){
  if(_previewAlbum){
    if(_previewAlbum.images.length<2)return;
    _previewState.index=(_previewState.index+1)%_previewAlbum.images.length;
    _previewImageData=_previewAlbum.images[_previewState.index];
    _resetAndLoad(_previewImageData.url);
    var raw=_previewImageData.prompt||'';var ni=raw.indexOf('\n--neg ');if(ni<0)ni=raw.indexOf('--neg ');
    if(raw){el('img-preview-meta-text').textContent=ni>=0?raw.substring(0,ni).trim():raw.trim();}
    else if(_previewImageData.path){el('img-preview-meta-text').textContent='⏳ 读取中...';var _gi=_previewImageData;var _aidx=_previewState.index;api('/api/bind/meta',{method:'POST',body:{path:_gi.path}}).then(function(d){if(d&&d.ok){var txt=d.positive||'';if(_previewAlbum&&_previewAlbum.images[_aidx]){_previewAlbum.images[_aidx].prompt=txt.trim();_saveAlbums();}el('img-preview-meta-text').textContent=txt.trim()||'（无）';}else el('img-preview-meta-text').textContent='（无）';}).catch(function(){el('img-preview-meta-text').textContent='（无）';});}
    else el('img-preview-meta-text').textContent='（无）';
    return;
  }
  if(_galleryImages.length<2)return;_previewState.index=(_previewState.index+1)%_galleryImages.length;_previewImageData=_galleryImages[_previewState.index];_resetAndLoad(_galleryImages[_previewState.index].url);
}
function _previewPrev(){
  if(_previewAlbum){
    if(_previewAlbum.images.length<2)return;
    _previewState.index=(_previewState.index-1+_previewAlbum.images.length)%_previewAlbum.images.length;
    _previewImageData=_previewAlbum.images[_previewState.index];
    _resetAndLoad(_previewImageData.url);
    var raw=_previewImageData.prompt||'';var ni=raw.indexOf('\n--neg ');if(ni<0)ni=raw.indexOf('--neg ');
    if(raw){el('img-preview-meta-text').textContent=ni>=0?raw.substring(0,ni).trim():raw.trim();}
  ...
  if(_galleryImages.length<2)return;_previewState.index=(_previewState.index-1+_galleryImages.length)%_galleryImages.length;_previewImageData=_galleryImages[_previewState.index];_resetAndLoad(_galleryImages[_previewState.index].url);
}
...
function _updatePreviewNav(){
  if(_previewAlbum){
    var show=_previewAlbum.images.length>1;
    el('img-preview-prev').style.display=show?'flex':'none';
    el('img-preview-next').style.display=show?'flex':'none';
  }else{
    el('img-preview-prev').style.display=_galleryImages.length>1?'flex':'none';
    el('img-preview-next').style.display=_galleryImages.length>1?'flex':'none';
  }
}
function _closeImgPreview(){_previewAlbum=null;el('img-preview-modal').classList.remove('active');el('img-preview-prompt-popup').style.display='none';}
...
function _popupCopyMeta(){
  var gi=_previewImageData;if(!gi)return;
...
/* ===== 相册功能 ===== */
var _albumData=[];var _activeAlbumId='default';
function _getActiveAlbumName(){for(var i=0;i<_albumData.length;i++)if(_albumData[i].id===_activeAlbumId)return _albumData[i].name;return'';}
function _loadAlbums(){try{_albumData=JSON.parse(localStorage.getItem('grimoire2_albums')||'[]');}catch(e){_albumData=[];}if(!_albumData.length||!_albumData.some(function(a){return a.id==='default';}))_albumData.unshift({id:'default',name:'相册',images:[]});else{for(var i=0;i<_albumData.length;i++)if(_albumData[i].id==='default'&&_albumData[i].name==='默认相册')_albumData[i].name='相册';}_activeAlbumId=localStorage.getItem('grimoire2_active_album')||'default';_renderAlbumTabs();_renderAlbumCard();}
function _saveAlbums(){localStorage.setItem('grimoire2_albums',JSON.stringify(_albumData));localStorage.setItem('grimoire2_active_album',_activeAlbumId);_syncSave({albums:1});}
function _renderAlbumTabs(){
  var list=el('album-list');if(!list)return;var html='';
  for(var i=0;i<_albumData.length;i++){var a=_albumData[i];var isActive=a.id===_activeAlbumId;
    html+='<div class="album-tab'+(isActive?' active':'')+'" data-aid="'+esc(a.id)+'">';
    html+='📁 <span class="tab-name">'+esc(a.name)+'</span><span class="tab-count">('+a.images.length+')</span>';
    html+='<span class="tab-heart" style="color:'+(isActive?'#f44':'var(--text-muted)')+';cursor:pointer">'+(isActive?'❤':'♡')+'</span>';
    if(a.id!=='default')html+='<button class="tab-del" data-aid="'+esc(a.id)+'">🗑</button>';
    html+='</div>';
  }
  list.innerHTML=html;
  // 点击标签切换活跃相册 / 打开卡片
  qsa('.album-tab',list).forEach(function(t){t.addEventListener('click',function(e){
    if(e.target.classList.contains('tab-del')||e.target.classList.contains('tab-heart'))return;
    var aid=this.dataset.aid;
    if(_activeAlbumId!==aid){_activeAlbumId=aid;_saveAlbums();_renderAlbumTabs();_refreshGalleryHearts();_renderAlbumCard();}
    else{_renderAlbumCard();} // 已活跃的点开卡片
  });});
  // ❤ 切换活跃
  qsa('.tab-heart',list).forEach(function(h){h.addEventListener('click',function(e){e.stopPropagation();var aid=this.parentElement.dataset.aid;_activeAlbumId=aid;_saveAlbums();_renderAlbumTabs();_refreshGalleryHearts();_renderAlbumCard();});});
  // 删除
  qsa('.tab-del',list).forEach(function(b){b.addEventListener('click',function(e){e.stopPropagation();var s=parseInt(this.dataset.strike||'0')+1;this.dataset.strike=s;if(s<5){this.textContent=s;this.style.color='var(--danger)';if(this._t)clearTimeout(this._t);var self=this;this._t=setTimeout(function(){self.dataset.strike='0';self.textContent='🗑';self.style.color='';},3000);}else{var aid=this.dataset.aid;_albumData=_albumData.filter(function(x){return x.id!==aid;});if(_activeAlbumId===aid)_activeAlbumId='default';_saveAlbums();_renderAlbumTabs();_renderAlbumCard();_refreshGalleryHearts();toast('相册已删除');}});});
}
// 相册卡片轮播
var _albumCardIdx=0;var _albumCardTimer=null;
function _renderAlbumCard(){
  var album=null;for(var i=0;i<_albumData.length;i++)if(_albumData[i].id===_activeAlbumId){album=_albumData[i];break;}
  var carousel=el('album-card-carousel');if(!carousel)return;
  carousel.style.display='block';
  _stopCardCarousel();
  var hasImg=album&&album.images.length>0;
  _albumCardIdx=0;
  el('album-card-img').src=hasImg?album.images[0].url:'';
  el('album-card-img').style.display=hasImg?'block':'none';
  el('album-card-name').textContent=album?album.name:'相册';
  el('album-card-count').textContent=hasImg?('1 / '+album.images.length):'空';
  el('album-card-prev').style.display=hasImg&&album.images.length>1?'flex':'none';
  el('album-card-next').style.display=hasImg&&album.images.length>1?'flex':'none';
  if(hasImg){el('album-card-img').onload=function(){_startCardCarousel(album);};}
  // 点击卡片打开相册管理弹窗
  el('album-card').onclick=function(){_openAlbumModal();};
}
function _startCardCarousel(album){_stopCardCarousel();if(!album||album.images.length<2)return;_albumCardTimer=setInterval(function(){var nxt;do{nxt=Math.floor(Math.random()*album.images.length);}while(nxt===_albumCardIdx&&album.images.length>1);_albumCardIdx=nxt;var img=album.images[_albumCardIdx];var cardImg=el('album-card-img');cardImg.style.opacity='0';var cardEl=el('album-card');cardEl.style.borderRadius='20px';setTimeout(function(){cardImg.src=img.url;cardImg.style.opacity='1';el('album-card-count').textContent=(_albumCardIdx+1)+' / '+album.images.length;setTimeout(function(){cardEl.style.borderRadius='14px';},400);},300);},4000);}
function _stopCardCarousel(){if(_albumCardTimer){clearInterval(_albumCardTimer);_albumCardTimer=null;}}
function _openCardImage(album,idx){if(!album||!album.images[idx])return;var gi=album.images[idx];_previewAlbum=album;_previewImageData=gi;_loadPreviewImage(gi.url);_previewState.index=idx;_previewState.scale=1;_previewState.panX=0;_previewState.panY=0;var raw=gi.prompt||'';if(raw){var ni=raw.indexOf('\n--neg ');if(ni<0)ni=raw.indexOf('--neg ');el('img-preview-meta-text').textContent=ni>=0?raw.substring(0,ni).trim():raw.trim();}else if(gi.path){el('img-preview-meta-text').textContent='⏳ 读取中...';api('/api/bind/meta',{method:'POST',body:{path:gi.path}}).then(function(d){if(d&&d.ok){var txt=d.positive||'';gi.prompt=txt.trim();album.images[idx]=gi;_saveAlbums();el('img-preview-meta-text').textContent=txt.trim()||'（无）';}else el('img-preview-meta-text').textContent='（无）';}).catch(function(){el('img-preview-meta-text').textContent='（无）';});}else el('img-preview-meta-text').textContent='（无）';el('img-preview-prompt-popup').style.display='none';el('img-preview-modal').classList.add('active');_updatePreviewNav();}
el('album-card-prev').addEventListener('click',function(e){e.stopPropagation();_stopCardCarousel();_albumCardIdx=Math.max(0,_albumCardIdx-1);var a=null;for(var i=0;i<_albumData.length;i++)if(_albumData[i].id===_activeAlbumId){a=_albumData[i];break;}if(a&&a.images[_albumCardIdx]){var gi=a.images[_albumCardIdx];el('album-card-img').src=gi.url;el('album-card-count').textContent=(_albumCardIdx+1)+' / '+a.images.length;}_startCardCarousel(a);});
el('album-card-next').addEventListener('click',function(e){e.stopPropagation();_stopCardCarousel();var a=null;for(var i=0;i<_albumData.length;i++)if(_albumData[i].id===_activeAlbumId){a=_albumData[i];break;}if(a){_albumCardIdx=(_albumCardIdx+1)%a.images.length;var gi=a.images[_albumCardIdx];el('album-card-img').src=gi.url;el('album-card-count').textContent=(_albumCardIdx+1)+' / '+a.images.length;}_startCardCarousel(a);});
// 画廊爱心按钮 - 保存/取消 切换
function _saveToAlbum(gi){if(!gi||!gi.url)return false;var album=null;for(var i=0;i<_albumData.length;i++)if(_albumData[i].id===_activeAlbumId){album=_albumData[i];break;}if(!album)return false;for(var i=0;i<album.images.length;i++)if(album.images[i].url===gi.url)return false;album.images.push({url:gi.url,filename:gi.filename||'',prompt:gi.prompt||'',path:gi.path||''});_saveAlbums();_renderAlbumTabs();_renderAlbumCard();return true;}
function _isImageInAnyAlbum(url){for(var j=0;j<_albumData.length;j++){for(var k=0;k<_albumData[j].images.length;k++){if(_albumData[j].images[k].url===url)return true;}}return false;}
function _removeFromAlbum(url){var album=null;for(var i=0;i<_albumData.length;i++)if(_albumData[i].id===_activeAlbumId){album=_albumData[i];break;}if(!album)return false;var rm=null;for(var i=0;i<album.images.length;i++)if(album.images[i].url===url){rm=album.images[i];break;}var before=album.images.length;album.images=album.images.filter(function(x){return x.url!==url;});if(album.images.length<before){_saveAlbums();_renderAlbumTabs();_renderAlbumCard();if(rm&&!_galleryImages.some(function(x){return x.url===url;})&&!_isImageInAnyAlbum(url)){_galleryImages.push(rm);_saveGallery();}_galleryReRender();return true;}return false;}
function _galleryReRender(){
  ...
  for(var i=0;i<_galleryImages.length;i++){
    ...
    var im=document.createElement('img');im.dataset.src=gi.url;im.style.background='var(--bg-tertiary)';
    im.onerror=function(){this.parentElement.style.display='none';};_observeGalImg(im);
...
// 相册管理弹窗
function _openAlbumModal(){_renderModalAlbumList();el('modal-album').style.display='';var mc=el('modal-album').querySelector('.modal-content');if(mc){mc.style.width='860px';mc.style.maxWidth='860px';mc.style.minWidth='860px';}el('album-new-name').focus();}
var _modalCoverTimers=[];
function _stopModalCoverTimers(){for(var i=0;i<_modalCoverTimers.length;i++)clearInterval(_modalCoverTimers[i]);_modalCoverTimers=[];}
function _renderModalAlbumList(){
  var div=el('album-modal-list');var html='';
  div.style.display='grid';div.style.gridTemplateColumns='repeat(auto-fill,180px)';div.style.gap='10px';
  for(var i=0;i<_albumData.length;i++){var a=_albumData[i];var isActive=a.id===_activeAlbumId;
    var coverImg=a.images.length?('background-image:url('+esc(a.images[Math.floor(Math.random()*a.images.length)].url)+');'):'';
    html+='<div style="display:flex;flex-direction:column;gap:4px;width:180px">';
    // 头部信息栏
    html+='<div style="display:flex;align-items:center;gap:6px">';
    html+='<span style="flex:1;font-size:12px;font-weight:600;color:var(--text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">📁 '+esc(a.name)+'</span>';
    html+='<span style="font-size:12px;color:var(--text-muted);white-space:nowrap">'+a.images.length+'张</span>';
    html+='<button class="album-modal-act" data-aid="'+esc(a.id)+'" style="font-size:10px;padding:2px 8px;border:1px solid '+(isActive?'var(--accent)':'var(--border)')+';border-radius:8px;background:'+(isActive?'rgba(139,124,247,0.2)':'transparent')+';color:'+(isActive?'var(--accent)':'var(--text-muted)')+';cursor:pointer;white-space:nowrap">'+(isActive?'❤活跃':'♡活跃')+'</button>';
    if(a.id!=='default')html+='<button class="album-modal-del" data-aid="'+esc(a.id)+'" style="border:none;background:none;color:var(--text-muted);cursor:pointer;font-size:15px;padding:0 3px">🗑</button>';
    html+='</div>';
    // 卡片（点击打开）
    html+='<div class="album-modal-card-click" data-aid="'+esc(a.id)+'" style="border:1px solid '+(isActive?'var(--accent)':'var(--border)')+';border-radius:8px;overflow:hidden;cursor:pointer;transition:box-shadow .2s">';
    html+='<div class="album-cover-rot" data-aid="'+esc(a.id)+'" style="aspect-ratio:9/16;'+coverImg+'background-size:cover;background-position:center;background-color:var(--bg-tertiary);position:relative;transition:background-image .6s">';
    if(!a.images.length)html+='<span style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:var(--text-muted);font-size:12px">空</span>';
    html+='</div></div></div>';
  }
  div.innerHTML=html;
  // 封面轮播定时器
  _stopModalCoverTimers();
  qsa('.album-cover-rot',div).forEach(function(cover){var aid=cover.dataset.aid;var album=null;for(var j=0;j<_albumData.length;j++)if(_albumData[j].id===aid){album=_albumData[j];break;}if(album&&album.images.length>1){var t=setInterval(function(){var nxt;do{nxt=Math.floor(Math.random()*album.images.length);}while(cover.style.backgroundImage.indexOf(album.images[nxt].url)>=0);cover.style.backgroundImage='url('+album.images[nxt].url+')';},4000);_modalCoverTimers.push(t);}});
  // 点击卡片打开相册浏览
  qsa('.album-modal-card-click',div).forEach(function(card){card.addEventListener('click',function(e){e.stopPropagation();_stopModalCoverTimers();el('modal-album').style.display='none';_openAlbumBrowse(this.dataset.aid);});});
  // 活跃按钮
  qsa('.album-modal-act',div).forEach(function(b){b.addEventListener('click',function(e){e.stopPropagation();e.stopImmediatePropagation();_activeAlbumId=this.dataset.aid;_saveAlbums();_stopModalCoverTimers();_renderModalAlbumList();_renderAlbumTabs();_renderAlbumCard();_refreshGalleryHearts();});});
  // 删除按钮
  qsa('.album-modal-del',div).forEach(function(b){b.addEventListener('click',function(e){e.stopPropagation();e.stopImmediatePropagation();var s=parseInt(this.dataset.strike||'0')+1;this.dataset.strike=s;if(s<5){this.textContent=s;this.style.color='var(--danger)';var self=this;if(this._t)clearTimeout(this._t);this._t=setTimeout(function(){self.dataset.strike='0';self.textContent='🗑';self.style.color='';},3000);}else{var aid=this.dataset.aid;_albumData=_albumData.filter(function(x){return x.id!==aid;});if(_activeAlbumId===aid)_activeAlbumId='default';_saveAlbums();_stopModalCoverTimers();_renderModalAlbumList();_renderAlbumTabs();_renderAlbumCard();_refreshGalleryHearts();toast('相册已删除');}});});
  // 双击重命名
  qsa('span[style*="font-weight:600"]',div).forEach(function(sp){sp.title='双击重命名';sp.style.cursor='text';sp.addEventListener('dblclick',function(e){e.stopPropagation();var parent=this.parentElement;var actBtn=parent.querySelector('.album-modal-act');var aid=actBtn?actBtn.dataset.aid:'';var old=this.textContent.replace(/^📁\s*/,'');var inp=document.createElement('input');inp.type='text';inp.value=old;inp.style.cssText='width:100%;padding:2px 4px;border:1px solid var(--accent);border-radius:3px;background:var(--bg-primary);color:var(--text-primary);font-size:12px;font-weight:600;outline:none';this.textContent='';this.appendChild(inp);inp.focus();inp.select();function _renameSave(){var nm=inp.value.trim();if(nm&&nm!==old){for(var j=0;j<_albumData.length;j++){if(_albumData[j].name===nm&&_albumData[j].id!==aid){toast('名称已存在');_renderModalAlbumList();return;}}for(var j=0;j<_albumData.length;j++){if(_albumData[j].id===aid){_albumData[j].name=nm;break;}}_saveAlbums();}_renderModalAlbumList();_renderAlbumTabs();_renderAlbumCard();}inp.addEventListener('blur',_renameSave);inp.addEventListener('keydown',function(ev){if(ev.key==='Enter'){ev.preventDefault();_renameSave();}});});});
}
// 批量删除全局状态
var _batchMode=false,_batchStrikes=0,_batchAlbum=null;
function _toggleBatchMode(){
  _batchMode=!_batchMode;
  var btn=el('btn-batch-del-confirm');
  btn.dataset.strike='0';btn.textContent='确认删除 (5次)';
  btn.style.borderColor='var(--accent)';btn.style.color='var(--accent)';btn.style.background='transparent';
  var tgl=el('btn-batch-del-toggle');
  el('btn-batch-del-confirm').style.display=_batchMode?'inline-block':'none';
  el('btn-batch-select-all').style.display=_batchMode?'inline-block':'none';
  el('btn-batch-send').style.display=_batchMode?'inline-block':'none';
  el('btn-batch-reverse').style.display=_batchMode?'inline-block':'none';
  if(_batchMode){
    tgl.style.borderColor='var(--danger)';tgl.style.color='var(--danger)';tgl.style.background='rgba(232,93,110,0.25)';
    _renderBrowseGrid(_batchAlbum,true);
  }else{
    tgl.style.borderColor='var(--accent)';tgl.style.color='var(--accent)';tgl.style.background='transparent';
    _renderBrowseGrid(_batchAlbum,false);
  }
}
function _executeBatchDel(){
  if(!_batchMode){toast('请先点击批量删除');return;}
  var cbs=el('album-browse-grid').querySelectorAll('.batch-cb:checked');var urls=[];
  cbs.forEach(function(cb){urls.push({url:cb.dataset.url,filename:cb.dataset.filename||'',prompt:cb.dataset.prompt||''});});
  if(!urls.length){toast('请勾选要删除的图片');return;}
  _batchAlbum.images=_batchAlbum.images.filter(function(x){return !urls.some(function(u){return u.url===x.url;});});
  _saveAlbums();_batchMode=false;
  urls.forEach(function(u){if(!_galleryImages.some(function(x){return x.url===u.url;})&&!_isImageInAnyAlbum(u.url)){_galleryImages.push(u);}});
  if(urls.length)_saveGallery();_galleryReRender();
  var tgl=el('btn-batch-del-toggle');tgl.style.borderColor='var(--accent)';tgl.style.color='var(--accent)';tgl.style.background='transparent';
  el('btn-batch-del-confirm').style.display='none';el('btn-batch-select-all').style.display='none';el('btn-batch-send').style.display='none';el('btn-batch-reverse').style.display='none';
  _renderBrowseGrid(_batchAlbum,false);_renderAlbumCard();_renderAlbumTabs();toast('已删除 '+urls.length+' 张');
}
function _openAlbumBrowse(aid){
  var album=null;for(var i=0;i<_albumData.length;i++)if(_albumData[i].id===aid){album=_albumData[i];break;}
  if(!album){return;}
  _batchAlbum=album;_batchMode=false;_batchStrikes=0;
  el('album-browse-title').textContent='📂 '+album.name+' ('+album.images.length+')';
  el('album-browse-search').value='';
  el('btn-batch-del-toggle').style.display='inline-block';
  el('btn-batch-del-confirm').style.display='none';
  el('btn-batch-select-all').style.display='none';
  el('btn-batch-send').style.display='none';
  el('btn-batch-reverse').style.display='none';
  _renderBrowseGrid(album,false);
  el('modal-album-browse').style.display='';
}
var _albumBrowseAid=null;
var _albumBrowseFilter='';
function _renderBrowseGrid(album,showDel){
  var grid=el('album-browse-grid');var html='';
  var filter=_albumBrowseFilter.toLowerCase();
  var sorted=_sortAlbumImages(album.images,_albumSortMode);
  var filtered=sorted;
  if(filter)filtered=sorted.filter(function(x){return(x.filename||'').toLowerCase().indexOf(filter)>=0||(x.prompt||'').toLowerCase().indexOf(filter)>=0;});
  if(!filtered.length){html='<div style="text-align:center;padding:40px;color:var(--text-muted);font-size:13px">'+(filter?'未找到匹配图片':'相册为空')+'</div>';}
  else{
    for(var i=0;i<sorted.length;i++){var gi=sorted[i];
      if(filter&&(gi.filename||'').toLowerCase().indexOf(filter)<0&&(gi.prompt||'').toLowerCase().indexOf(filter)<0)continue;
      html+='<div class="album-grid-card" data-idx="'+i+'" data-url="'+esc(gi.url)+'" data-filename="'+esc(gi.filename||'')+'" data-prompt="'+esc(gi.prompt||'')+'" data-path="'+esc(gi.path||'')+'">';
      html+='<div style="position:relative;height:100%"><img data-src="'+esc(gi.url)+'" style="height:200px;width:auto;display:block;object-fit:contain;border-radius:6px;background:var(--bg-tertiary)">';
      if(showDel)html+='<input type="checkbox" class="batch-cb" data-url="'+esc(gi.url)+'" data-filename="'+esc(gi.filename||'')+'" data-prompt="'+esc(gi.prompt||'')+'" style="position:absolute;top:4px;right:4px;width:20px;height:20px;cursor:pointer;accent-color:var(--danger)" onclick="event.stopPropagation()">';
      else html+='<button class="grid-del-btn" data-url="'+esc(gi.url)+'" style="position:absolute;top:4px;right:4px;background:rgba(0,0,0,0.5);border:none;color:#fff;font-size:11px;cursor:pointer;padding:2px 6px;border-radius:4px" title="删除(需2次)">🗑</button>';
      html+='</div>';
      html+='<div class="grid-label">'+(gi.filename||'#'+(i+1))+'</div>';
      html+='</div>';
    }
  }
  grid.innerHTML=html;
  // 相册浏览懒加载
  qsa('#album-browse-grid img[data-src]',grid).forEach(function(img){_observeGalImg(img);});
  // 单张删除按钮（2次确认）
  qsa('.grid-del-btn',grid).forEach(function(b){b.addEventListener('click',function(e){e.stopPropagation();var url=this.dataset.url;var s=parseInt(this.dataset.strike||'0')+1;this.dataset.strike=s;if(s<2){this.textContent='🔥';this.style.background='rgba(232,93,110,0.6)';var self=this;if(this._t)clearTimeout(this._t);this._t=setTimeout(function(){self.dataset.strike='0';self.textContent='🗑';self.style.background='rgba(0,0,0,0.5)';},2000);}else{var rmData={url:url,filename:this.dataset.filename||'',prompt:'',path:''};album.images=album.images.filter(function(x){return x.url!==url;});_saveAlbums();if(!_galleryImages.some(function(x){return x.url===url;})&&!_isImageInAnyAlbum(url)){_galleryImages.push(rmData);_saveGallery();}_galleryReRender();_renderBrowseGrid(album,false);_renderAlbumCard();_renderAlbumTabs();toast('已移除');}});});
  // 点击卡片打开大图（批量模式下不打开）
  qsa('.album-grid-card',grid).forEach(function(card){card.addEventListener('click',function(e){if(e.target.tagName==='INPUT'||e.target.classList.contains('grid-del-btn'))return;if(showDel)return;var gi={url:this.dataset.url,filename:this.dataset.filename,prompt:this.dataset.prompt,path:this.dataset.path||''};var giOrig=null;for(var i=0;i<album.images.length;i++)if(album.images[i].url===gi.url){giOrig=album.images[i];break;}_previewAlbum=album;_previewImageData=gi;_loadPreviewImage(gi.url);_previewState.index=parseInt(this.dataset.idx)||0;_previewState.scale=1;_previewState.panX=0;_previewState.panY=0;var raw=gi.prompt||'';if(raw){var ni=raw.indexOf('\n--neg ');if(ni<0)ni=raw.indexOf('--neg ');el('img-preview-meta-text').textContent=ni>=0?raw.substring(0,ni).trim():raw.trim();}else if(gi.path){el('img-preview-meta-text').textContent='⏳ 读取中...';api('/api/bind/meta',{method:'POST',body:{path:gi.path}}).then(function(d){if(d&&d.ok){var txt=d.positive||'';if(giOrig)giOrig.prompt=txt.trim();_saveAlbums();el('img-preview-meta-text').textContent=txt.trim()||'（无）';}else el('img-preview-meta-text').textContent='（无）';}).catch(function(){el('img-preview-meta-text').textContent='（无）';});}else el('img-preview-meta-text').textContent='（无）';el('img-preview-prompt-popup').style.display='none';el('img-preview-modal').classList.add('active');_updatePreviewNav();});});
}
var _abSearchTimer;el('album-browse-search').addEventListener('input',function(){_albumBrowseFilter=this.value;clearTimeout(_abSearchTimer);_abSearchTimer=setTimeout(function(){_renderBrowseGrid(_batchAlbum,_batchMode);},300);});
var _albumSortMode='name_asc';var _sortModes=['name_asc','name_desc','added_desc','added_asc','size_desc','size_asc','width_desc','width_asc'];var _sortLabels={'name_asc':'🔤 文件名↓','name_desc':'🔤 文件名↑','added_desc':'🕐 最新↑','added_asc':'🕐 最早↑','size_desc':'📐 大图↑','size_asc':'📐 小图↑','width_desc':'↔ 最宽↑','width_asc':'↔ 最窄↑'};var _dimCache={};
var _dimSortModes={size_desc:1,size_asc:1,width_desc:1,width_asc:1};
el('btn-album-sort').addEventListener('click',function(e){e.stopPropagation();var btn=this;var existing=document.getElementById('sort-dropdown');if(existing){existing.remove();return;}var div=document.createElement('div');div.id='sort-dropdown';div.style.cssText='position:absolute;top:100%;left:0;z-index:999;background:var(--bg-secondary);border:1px solid var(--border);border-radius:6px;padding:4px;box-shadow:var(--shadow-lg);min-width:140px';var html='';_sortModes.forEach(function(m){var act=m===_albumSortMode?' active':'';html+='<div class="sort-dropdown-item'+act+'" data-mode="'+m+'" style="padding:6px 12px;cursor:pointer;border-radius:4px;font-size:12px;transition:background .1s">'+_sortLabels[m]+'</div>';});div.innerHTML=html;btn.style.position='relative';btn.appendChild(div);qsa('.sort-dropdown-item',div).forEach(function(it){it.addEventListener('click',function(ev){ev.stopPropagation();div.remove();var mode=this.dataset.mode;_albumSortMode=mode;btn.textContent=_sortLabels[mode]||'🔤 文件名↓';if(_batchAlbum){if(_dimSortModes[mode]){_loadAllDims(_batchAlbum.images,function(){_renderBrowseGrid(_batchAlbum,_batchMode);});}else{_renderBrowseGrid(_batchAlbum,_batchMode);}}});});document.addEventListener('click',function _closeSort(){var d=document.getElementById('sort-dropdown');if(d)d.remove();document.removeEventListener('click',_closeSort);},{once:true});});
...
el('btn-album-browse-close').addEventListener('click',function(){el('modal-album-browse').style.display='none';});
el('modal-album-browse').addEventListener('click',function(e){if(e.target===this)this.style.display='none';});
el('btn-album-create').addEventListener('click',function(){var nm=el('album-new-name').value.trim();if(!nm){toast('请输入相册名称');return;}for(var i=0;i<_albumData.length;i++){if(_albumData[i].name===nm){toast('相册名已存在');return;}}_albumData.push({id:'album_'+Date.now(),name:nm,images:[]});_saveAlbums();_renderAlbumTabs();_renderAlbumCard();_renderModalAlbumList();el('album-new-name').value='';toast('相册已创建');});
el('btn-album-close').addEventListener('click',function(){el('modal-album').style.display='none';});
el('modal-album').addEventListener('click',function(e){if(e.target===this)this.style.display='none';});
// small + button on tabs also opens modal
el('btn-album-create-small').addEventListener('click',function(){_openAlbumModal();});
// 批量导入图片到当前相册
el('btn-album-import').addEventListener('click',function(){var bp=[];try{bp=JSON.parse(localStorage.getItem('grimoire2_bp')||'[]');}catch(e){}el('bind-path-1').value=bp[0]||'';el('bind-path-2').value=bp[1]||'';el('btn-bind-scan').textContent='📥 扫描导入';_importMode=Date.now();el('modal-bind-path').style.display='';});
// 初始化
_loadAlbums();
// 底部区域初始高度
el('bottom-area').style.height=(localStorage.getItem('grimoire2_bottom_h')||220)+'px';

// 可拖动调整底部区域高度
(function(){
  var handle=el('resize-handle');if(!handle)return;
  var panel=el('bottom-area');var dragging=false,startY,startH;
  handle.addEventListener('mousedown',function(e){dragging=true;startY=e.clientY;startH=panel.offsetHeight;document.body.style.cursor='ns-resize';document.body.style.userSelect='none';e.preventDefault();});
  document.addEventListener('mousemove',function(e){if(!dragging)return;var dh=e.clientY-startY;var newH=Math.max(100,Math.min(500,startH-dh));panel.style.height=newH+'px';localStorage.setItem('grimoire2_bottom_h',newH);});
  ...
  // 触摸支持
  handle.addEventListener('touchstart',function(e){dragging=true;startY=e.touches[0].clientY;startH=panel.offsetHeight;e.preventDefault();});
  document.addEventListener('touchmove',function(e){if(!dragging)return;var dh=e.touches[0].clientY-startY;var newH=Math.max(100,Math.min(500,startH-dh));panel.style.height=newH+'px';localStorage.setItem('grimoire2_bottom_h',newH);});
...
function _initMobile(){
  ...
  // 手机搜索
  var _mSearchTimer;el('search-input-m').addEventListener('input',function(){
    var q=this.value.trim();
    el('search-input').value=q;
    var b=el('btn-search-clear-m');
    if(q){b.style.display='inline-block';clearTimeout(_mSearchTimer);_mSearchTimer=setTimeout(function(){doSearch(q);},300);}
    else{b.style.display='none';clearTimeout(_mSearchTimer);clearSearch();}
  });
...
(function(){
  ...
  menuBtn.innerHTML=icon('folder')+'选择分类';
...
// 手机端独立初始化（避免被 _initMobile 的 try/catch 打断）
function _initMobileUI(){
  ...
  try{
    ...
    el('btn-fullscreen').addEventListener('click',function(){
      try{
        if(document.fullscreenElement||document.webkitFullscreenElement){
          ...
          if(de.requestFullscreen){de.requestFullscreen().catch(function(e){toast('全屏失败:'+e.message);});}
          else if(de.webkitRequestFullscreen){de.webkitRequestFullscreen();}
          else{toast('当前浏览器不支持全屏');}
        }
      }catch(e){toast('全屏出错:'+e.message);}
    });
    // 免责声明
    ...
    // 标签分类按钮
    var oldMenu=document.getElementById('m-menu-btn');
    if(!oldMenu){
      var menuBtn=document.createElement('button');
      menuBtn.id='m-menu-btn';
      menuBtn.style.cssText='flex-shrink:0;font-size:12px;padding:8px 14px;border:1px solid var(--border);border-radius:6px;background:var(--bg-primary);color:var(--text-primary);cursor:pointer;white-space:nowrap';
  ...
  }catch(e){console.error('MobileUI:',e);}
}
_initMobileUI();
...
function _buildMobileDrawer(){
  ...
  for(var i=0;i<S.allData.categories.length;i++){
    ...
    html+='<button class=m-cat-mode-btn data-cat="'+esc(c.name)+'" style="padding:2px 6px;border:1px solid var(--border);border-radius:4px;background:var(--bg-primary);color:var(--text-muted);font-size:13px;cursor:pointer;margin-right:4px" title='+(cm==='cat'?'按分类随机':'按子类随机')+'>'+(cm==='cat'?'<span class="svg-icon"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg></span> 分类':'<span class="svg-icon"><svg viewBox="0 0 24 24"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg></span> 子类')+'</button>';
  ...
  qsa('.m-cat-mode-btn',list).forEach(function(b){
    b.addEventListener('click',function(e){
      ...
      saveCatModes();
      this.innerHTML=S.catModes[cat]==='cat'?'<span class="svg-icon"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg></span> 分类':'<span class="svg-icon"><svg viewBox="0 0 24 24"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg></span> 子类';
...
function _updateMobileSelected(){
  ...
  for(var i=0;i<all.length;i++){
    var t=all[i];
    var locked=t.locked;
    html+='<div class=selected-chip style="display:flex;align-items:center;gap:4px;padding:6px 10px;border-radius:12px;background:var(--tag-selected-bg);border:1px solid '+(locked?'#e6a848':'var(--accent)')+';font-size:11px" data-en="'+esc(t.en)+'" data-panel=positive>';
    html+='<span class=chip-lock data-en="'+esc(t.en)+'" data-panel=positive style="font-size:14px;cursor:pointer;display:flex;'+(locked?'color:#e6a848;':'color:#5e6078;')+'" title="'+(locked?'已锁定-点击解锁':'点击锁定')+'">'+(locked?icon('lock'):icon('unlock'))+'</span>';
